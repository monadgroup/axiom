...
bool CreateControlAction::forward(bool) {
    switch (type) {
        ...
        case Control::ControlType::MIDI_SCALAR:
            root()->pool().registerObj(MidiControl::create(uuid, parentUuid, QPoint(0, 0), QSize(0, 0), false, name, root()));
            break;
        case Control::ControlType::NUM_EXTRACT:
            root()->pool().registerObj(ExtractControl::create(uuid, parentUuid, QPoint(0, 0), QSize(2, 2), false, name, ConnectionWire::WireType::NUM, 0, root()));
            break;
        case Control::ControlType::MIDI_EXTRACT:
            root()->pool().registerObj(ExtractControl::create(uuid, parentUuid, QPoint(0, 0), QSize(2, 2), false, name, ConnectionWire::WireType::MIDI, 0, root()));
            break;
        default: unreachable;
    }
    return false;
