...
namespace AxiomModel {
    ...
    public:
        ...
        static std::unique_ptr<AutomationNode> create(const QUuid &uuid, const QUuid &parentUuid, QPoint pos,
                                                      QSize size, bool selected, QString name,
                                                      const QUuid &controlsUuid, ModelRoot *root);

        static std::unique_ptr<AutomationNode> deserialize(QDataStream &stream, const QUuid &uuid,
