...
fn const_to_num(val: &ConstantValue, range: &SourceRange) -> CompileResult<ConstantNum> {
    match val.as_num() {
        ...
        None => Err(CompileError::mismatched_type(VarType::Num, VarType::of_constant(val), *range))
    }
}

fn consts_to_nums<'a>(vals: impl Iterator<Item = &'a ConstantValue>, range: &SourceRange) -> CompileResult<Vec<ConstantNum>> {
    vals.map(|val| { const_to_num(val, range) })
        .collect()
}

fn const_num_intrinsic(arg: &ConstantValue, range: &SourceRange, f: &Fn(f32) -> f32) -> CompileResult<ConstantValue> {
    const_to_num(arg, range).and_then(|num| {
        Ok(ConstantValue::new_num(f(num.left), f(num.right), num.form))
    })
}

fn two_const_num_intrinsic(a: &ConstantValue, b: &ConstantValue, range: &SourceRange, f: &Fn(f32, f32) -> f32) -> CompileResult<ConstantValue> {
    const_to_num(a, range).and_then(|a_num| {
        const_to_num(b, range).and_then(|b_num| {
            Ok(ConstantValue::new_num(f(a_num.left, b_num.left), f(a_num.right, b_num.right), a_num.form))
        })
    })
}
...
pub fn const_call(
    ...
    match function {
        ...
        Function::Atan2 => Some(two_const_num_intrinsic(&args[0], &args[1], range, &|a, b| { a.atan2(b) })),
        Function::Hypot => Some(two_const_num_intrinsic(&args[0], &args[1], range, &|a, b| { a.hypot(b) })),
        ...
        Function::Clamp => {
            Some(const_to_num(&args[0], range).and_then(|x| {
                const_to_num(&args[1], range).and_then(|min| {
                    const_to_num(&args[2], range).and_then(|max| {
                        Ok(ConstantValue::new_num(x.left.max(min.left).min(max.left), x.right.max(min.right).min(max.right), x.form))
                    })
                })
            }))
        },
        Function::Pan => {
            Some(const_to_num(&args[0], range).and_then(|val| {
                const_to_num(&args[1], range).and_then(|pan| {
                    ...
                    let left_amt = ((1. - left_pan) * (consts::PI / 4. * (left_pan + 1.)).cos() / 2.).sqrt();

                    let right_pan = pan.right.max(-1.).min(1.);
                    let right_amt = ((1. + right_pan) * (consts::PI / 4. * (right_pan + 1.)).sin() / 2.).sqrt();

                    Ok(ConstantValue::new_num(val.left * left_amt, val.right * right_amt, val.form))
                })
            }))
        },
        Function::Left => Some(const_to_num(&args[0], range).and_then(|num| { Ok(ConstantValue::new_num(num.left, num.left, num.form)) })),
        Function::Right => Some(const_to_num(&args[0], range).and_then(|num| { Ok(ConstantValue::new_num(num.right, num.right, num.form)) })),
        Function::Swap => Some(const_to_num(&args[0], range).and_then(|num| { Ok(ConstantValue::new_num(num.right, num.left, num.form)) })),
        Function::Combine => {
            Some(const_to_num(&args[0], range).and_then(|left| {
                const_to_num(&args[1], range).and_then(|right| {
                    Ok(ConstantValue::new_num(left.left, right.right, left.form))
                })
            }))
        },
        Function::Mix => {
            Some(const_to_num(&args[0], range).and_then(|a| {
        ...
        Function::Sequence => {
            Some(consts_to_nums(args.iter().chain(varargs.iter()), range)
                .and_then(|args| {
        ...
        Function::Min => {
            Some(consts_to_nums(args.iter().chain(varargs.iter()), range)
                .and_then(|args| {
                    let (new_left, new_right) = args.iter().skip(1).fold((args[0].left, args[0].right), |(acc_l, acc_r), num| {
