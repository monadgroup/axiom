...
pub fn gen_math_op_statement(op: &OperatorType, lhs: usize, rhs: usize, node: &mut NodeContext) -> PointerValue {
    ...
    let result_vec = util::float2vec(match op {
        ...
        OperatorType::Multiply => node.builder.build_float_mul(&left_vec, &right_vec, "num.mul.vec"),
        OperatorType::Divide => node.builder.build_float_div(&left_vec, &right_vec, "num.divide.vec"),
        OperatorType::Modulo => node.builder.build_float_rem(&left_vec, &right_vec, "num.mod.vec"),
        OperatorType::Power => unimplemented!(),
        OperatorType::BitwiseAnd => apply_int_op(node, &left_vec, &right_vec, &node.context.i32_type(), &|lhs, rhs| node.builder.build_and(lhs, rhs, "num.and.vec")),
        OperatorType::BitwiseOr => apply_int_op(node, &left_vec, &right_vec, &node.context.i32_type(), &|lhs, rhs| node.builder.build_or(lhs, rhs, "num.or.vec")),
        OperatorType::BitwiseXor => apply_int_op(node, &left_vec, &right_vec, &node.context.i32_type(), &|lhs, rhs| node.builder.build_xor(lhs, rhs, "num.xor.vec")),
        OperatorType::LogicalAnd => apply_int_op(node, &left_vec, &right_vec, &node.context.bool_type(), &|lhs, rhs| node.builder.build_and(lhs, rhs, "num.and.vec")),
        OperatorType::LogicalOr => apply_int_op(node, &left_vec, &right_vec, &node.context.bool_type(), &|lhs, rhs| node.builder.build_or(lhs, rhs, "num.or.vec")),
        OperatorType::LogicalEqual => int_to_float_vec(node, &node.builder.build_float_compare(FloatPredicate::OEQ, &left_vec, &right_vec, "num.vec.equal")),
        OperatorType::LogicalNotEqual => int_to_float_vec(node, &node.builder.build_float_compare(FloatPredicate::ONE, &left_vec, &right_vec, "num.vec.notequal")),
        OperatorType::LogicalGt => int_to_float_vec(node, &node.builder.build_float_compare(FloatPredicate::OGT, &left_vec, &right_vec, "num.vec.gt")),
