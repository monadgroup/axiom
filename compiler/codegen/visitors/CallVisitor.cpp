...
std::unique_ptr<Value> MaximCodegen::visitCall(ComposableModuleClassMethod *method, Scope *scope, MaximAst::CallExpression *expr) {
    ...
    return method->moduleClass()->ctx()->callFunction(expr->name, std::move(argVals), method, expr->startPos, expr->endPos);
}
