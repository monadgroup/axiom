...
std::unique_ptr<Value> AmplitudeFunction::generate(Builder &b, std::vector<std::unique_ptr<Value>> params,
    ...
    auto absIntrinsic = llvm::Intrinsic::getDeclaration(module, llvm::Intrinsic::ID::fabs, {context()->numType()->vecType()});
    ...
    return paramVal->withVec(b, newEstimate, undefPos, undefPos)->withForm(b, MaximCommon::FormType::LINEAR, undefPos, undefPos);
}
